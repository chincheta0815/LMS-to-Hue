...
void _scale_and_pack_frames(void *outputptr, s32_t *inputptr, frames_t cnt, s32_t gainL, s32_t gainR, u8_t flags, output_format format) {
	...
	if ((flags & MONO_LEFT) && (flags & MONO_RIGHT)) {
		...
		frames_t count = cnt;
		while (count--) {
			// use 64 bit integers for purists but should really not care
	...
	switch (format) {
		...
		case S24_3LE: {
				...
				if (gainL == FIXED_ONE && gainR == FIXED_ONE) {
					while(cnt) {
						// attempt to do 32 bit memory access - move 2 frames at once: 16 bytes -> 12 bytes
						...
						if (((uintptr_t)optr &0x3) == 0 && cnt >= 2) {
							...
							while (cnt >= 2) {
								...
								*(o_ptr++) = (l1 & 0x0000ff00) << 16 | (l1 & 0x00ff0000) | (l1 & 0xff000000) >> 16 |
									(r1 & 0x0000ff00) >>  8;
								*(o_ptr++) = (r1 & 0x00ff0000) <<  8 | (r1 & 0xff000000) | (l2 & 0x0000ff00) |
									(l2 & 0x00ff0000) >> 16;
				...
				if (gainL == FIXED_ONE && gainR == FIXED_ONE) {
					...
					while (cnt--) {
						...
						*(optr++) =
							...
							(lsample & 0x0000ff00) <<  8 | (lsample & 0x000000ff) << 24;
						*(optr++) =
							(rsample & 0xff000000) >> 24 | (lsample & 0x00ff0000) >>  8 |
							(rsample & 0x0000ff00) <<  8 | (rsample & 0x000000ff) << 24;	
					}
				}
#endif
